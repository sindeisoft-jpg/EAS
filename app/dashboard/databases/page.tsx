...
export default function DatabasesPage() {
  ...
  const handleDelete = async (id: string) => {
    ...
    const confirmed = await showDangerConfirm(
      "删除数据源",
      `确定要删除数据源 "${connection?.name || "未知"}" 吗？此操作不可恢复。`,
      async () => {
    ...
    <div className="p-6 overflow-y-auto h-full">
      <div className="max-w-7xl mx-auto">
        <div className="flex items-center justify-between mb-8">
          <div>
            <h1 className="text-3xl font-bold text-foreground mb-2 tracking-tight">数据源管理</h1>
            <p className="text-sm text-muted-foreground">管理您的数据源连接以进行AI查询</p>
          </div>
          <Button 
        ...
        {connections.length === 0 ? (
          <EmptyState
            icon={Database}
            title="暂无数据源"
            description="开始添加您的第一个数据源以进行AI查询"
            action={{
              label: "添加第一个数据库",
              onClick: () => setIsDialogOpen(true),
            }}
