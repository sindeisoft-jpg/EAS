...
import { Plus, Eraser, Loader2 } from "lucide-react"
...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  const [localSearchInput, setLocalSearchInput] = useState("") // 本地输入状态，不立即触发搜索
  ...
  // 监听 chat-interface 的搜索查询变化
  useEffect(() => {
    const handleStorageChange = () => {
      const stored = localStorage.getItem("chat_search_query")
      if (stored !== null && stored !== chatSearchQuery) {
        setChatSearchQuery(stored)
      }
    }
  ...
  // 同步 chatSearchQuery 到本地输入状态（当从外部更新时）
  useEffect(() => {
    setLocalSearchInput(chatSearchQuery)
  }, [chatSearchQuery])
  
  // 同步搜索查询到 chat-interface
  useEffect(() => {
    if (chatSearchQuery !== undefined) {
      localStorage.setItem("chat_search_query", chatSearchQuery)
      // 触发自定义事件通知 chat-interface
      window.dispatchEvent(new CustomEvent("chat-search-query-change", { detail: chatSearchQuery }))
    }
  }, [chatSearchQuery])
  
  // 监听 chat-interface 的状态变化
  useEffect(() => {
    const handleCreatingSession = () => {
    ...
    const handleClearingAll = () => {
      ...
      setTimeout(() => setIsClearingAll(false), 1000)
    }
    
    window.addEventListener("chat-creating-session", handleCreatingSession as EventListener)
    window.addEventListener("chat-clearing-all", handleClearingAll as EventListener)
    
    return () => {
      window.removeEventListener("chat-creating-session", handleCreatingSession as EventListener)
      window.removeEventListener("chat-clearing-all", handleClearingAll as EventListener)
    }
  }, [])
  
  // 创建新会话
  const handleCreateNewSession = useCallback(() => {
    window.dispatchEvent(new CustomEvent("chat-create-new-session"))
  }, [])
  
  // 清空所有聊天记录
  const handleClearAll = useCallback(() => {
    window.dispatchEvent(new CustomEvent("chat-clear-all"))
  }, [])
  ...
  const navigation = [
    ...
    { name: "数据源管理", href: "/dashboard/databases", icon: Database },
    { name: "大模型管理", href: "/dashboard/models", icon: Brain },
    { name: "智能体市场", href: "/dashboard/marketplace", icon: Store },
      ...
      <div className={`transition-all duration-300 flex flex-col h-screen ${isMobile ? "ml-0" : isCollapsed ? "ml-16" : "ml-72"}`}>
        ...
        <header className="sticky top-0 z-50 w-full bg-background/95 backdrop-blur-xl supports-[backdrop-filter]:bg-background/90 border-b border-border/50 relative flex-shrink-0">
          <div className="flex h-14 items-center justify-between px-4 md:px-6 gap-4">
            ...
            {/* 中间：对话搜索和操作按钮 - 仅在 /dashboard 路径显示，绝对居中 */}
            {pathname === "/dashboard" && (
              <div className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 flex items-center gap-2">
                <div className="relative">
                  ...
                  <input
                    ...
                    onKeyDown={(e) => {
                      if (e.key === "Enter") {
                        e.preventDefault()
                        setChatSearchQuery(localSearchInput)
                      }
                    }}
                    className="w-64 h-9 pl-9 pr-3 rounded-lg bg-[#f9fafb] border border-[#e5e7eb] focus:border-[#1e3a5f] focus:ring-2 focus:ring-[#1e3a5f]/20 text-sm transition-all duration-300 hover:bg-[#f3f4f6] text-[#374151]"
                  />
                </div>
                <Button
                  ...
                  size="icon"
                  className="h-9 w-9 rounded-lg bg-gradient-to-br from-[#1e3a5f] to-[#2c5282] text-white hover:from-[#2c5282] hover:to-[#1e3a5f] transition-all duration-300 flex-shrink-0 shadow-md hover:shadow-lg hover:scale-105 disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:scale-100"
                ...
                <Button
                  ...
                  size="icon"
                  className="h-9 w-9 rounded-lg bg-white border border-[#e5e7eb] text-[#6b7280] hover:bg-[#f3f4f6] hover:text-[#dc2626] hover:border-[#dc2626]/30 transition-all duration-300 flex-shrink-0 shadow-sm hover:shadow-md hover:scale-105"
                  title="清空所有聊天记录"
