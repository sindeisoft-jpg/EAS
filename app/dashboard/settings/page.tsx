...
import { Settings, Database, Shield, Bell, Zap, Loader2, Eye, EyeOff, Download, RefreshCw, Server, Pencil } from "lucide-react"
...
export default function SettingsPage() {
  ...
  // 本系统自身使用的数据库（安装时配置的 DATABASE_URL，用于存储用户、组织、设置等）
  ...
  const [systemDbConfigured, setSystemDbConfigured] = useState(false)
  const [systemDbLoading, setSystemDbLoading] = useState(false)
  const [systemDbEditing, setSystemDbEditing] = useState(false)
  const [systemDbForm, setSystemDbForm] = useState<{ host: string; port: number; database: string; username: string; password: string }>({
    host: "",
    port: 3306,
    database: "",
    username: "",
  ...
  const [systemDbSaving, setSystemDbSaving] = useState(false)
  ...
  useEffect(() => {
    const loadSystemDatabase = async () => {
      if (user?.role !== "admin") return
      setSystemDbLoading(true)
      try {
        const data = await apiClient.getSystemDatabase()
        setSystemDbConfigured(data.configured)
        setSystemDbConfig(data.config)
        if (data.config) {
          setSystemDbForm({
        ...
        console.error("Failed to load system database config:", error)
        setSystemDbConfig(null)
        setSystemDbConfigured(false)
      } finally {
        setSystemDbLoading(false)
      }
    }
    loadSystemDatabase()
  }, [user])

  const startEditSystemDb = () => {
    if (systemDbConfig) {
      setSystemDbForm({
        host: systemDbConfig.host,
        port: systemDbConfig.port,
        database: systemDbConfig.database,
        username: systemDbConfig.username,
        password: "",
      })
    }
    setSystemDbEditing(true)
  }

  const cancelEditSystemDb = () => {
    setSystemDbEditing(false)
  }

  const handleSaveSystemDb = async () => {
    if (!systemDbForm.host || !systemDbForm.database || !systemDbForm.username || !systemDbForm.password) {
      alert("请填写主机、数据库名、用户名和密码")
      return
    }
    setSystemDbSaving(true)
    try {
      const res = await apiClient.updateSystemDatabase({
        host: systemDbForm.host,
        port: Number(systemDbForm.port) || 3306,
        database: systemDbForm.database,
        username: systemDbForm.username,
        password: systemDbForm.password,
      })
      alert(res.message)
      if (res.config) {
        setSystemDbConfig(res.config)
        setSystemDbConfigured(true)
        setSystemDbForm((f) => ({ ...f, password: "" }))
      }
      setSystemDbEditing(false)
    } catch (error: any) {
      const msg = error.message || (error.originalError?.message ?? "未知错误")
      const hint = error.hint ? `\n${error.hint}` : ""
      alert("保存失败: " + msg + hint)
    } finally {
      setSystemDbSaving(false)
    }
  }
        ...
        <div className="flex items-center justify-between mb-8">
          ...
          <div className="flex items-center gap-2">
            <Button
              variant="outline"
              className="gap-2"
              onClick={() => router.push("/install?reinstall=1")}
              title="进入数据库安装与配置向导"
            >
        ...
        <Tabs defaultValue="database" className="space-y-6">
          ...
          <TabsContent value="database">
            <Card className="p-6 space-y-6">
              <div>
                <h3 className="text-lg font-semibold text-foreground mb-2">本系统数据库配置</h3>
                <p className="text-sm text-muted mb-4">
                  即通过「安装」或「重新安装」配置的数据库，用于存储本系统的用户、组织、设置等数据（与对话/分析用的业务库无关）。可在此查看并修改连接参数，保存后需重启应用服务生效。
                </p>
                {systemDbLoading ? (
                  <div className="flex items-center justify-center py-8">
                    <Loader2 className="w-6 h-6 animate-spin" />
                  </div>
                ) : !systemDbConfigured || !systemDbConfig ? (
                  <div className="rounded-lg border border-dashed border-border bg-muted/30 p-8 text-center text-muted-foreground">
                    <Server className="w-10 h-10 mx-auto mb-2 opacity-50" />
                    <p>尚未配置本系统数据库</p>
                    <p className="text-sm mt-1">请通过「安装」或「重新安装」向导完成系统数据库配置</p>
                    <Button
                      variant="outline"
                  ...
                  <div className="space-y-4">
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                      <div className="space-y-2">
                        <Label>主机</Label>
                        <Input
                          value={systemDbForm.host}
                          onChange={(e) => setSystemDbForm({ ...systemDbForm, host: e.target.value })}
                      ...
                      <div className="space-y-2">
                        ...
                        <Input
                          type="number"
                          value={systemDbForm.port}
                          onChange={(e) => setSystemDbForm({ ...systemDbForm, port: Number(e.target.value) || 3306 })}
                          placeholder="3306"
                        />
                      </div>
                      <div className="space-y-2">
                        <Label>数据库名</Label>
                        <Input
                          value={systemDbForm.database}
                          onChange={(e) => setSystemDbForm({ ...systemDbForm, database: e.target.value })}
                          placeholder="eas"
                        />
                      </div>
                      <div className="space-y-2">
                        <Label>用户名</Label>
                        <Input
                          value={systemDbForm.username}
                          onChange={(e) => setSystemDbForm({ ...systemDbForm, username: e.target.value })}
                          placeholder="root"
                        />
                      </div>
                      <div className="space-y-2 sm:col-span-2">
                        ...
                        <Input
                          type="password"
                          value={systemDbForm.password}
                          onChange={(e) => setSystemDbForm({ ...systemDbForm, password: e.target.value })}
                          placeholder="必填，用于保存到 .env"
                          autoComplete="new-password"
                        />
                      </div>
                    </div>
                    <div className="flex items-center gap-2">
                      <Button onClick={handleSaveSystemDb} disabled={systemDbSaving}>
                        {systemDbSaving ? <><Loader2 className="w-4 h-4 animate-spin mr-1" /> 保存中...</> : "保存配置"}
                      </Button>
                      <Button variant="outline" onClick={cancelEditSystemDb}>
                        取消
                      </Button>
                    </div>
                  </div>
                  ...
                  <>
                    <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 text-sm rounded-lg border bg-muted/20 p-4">
                      <div>
                        <span className="text-muted-foreground">类型</span>
                        <p className="font-medium">{systemDbConfig.type.toUpperCase()}</p>
                      </div>
                      <div>
                        <span className="text-muted-foreground">主机</span>
                        <p className="font-medium">{systemDbConfig.host}:{systemDbConfig.port}</p>
                      </div>
                      <div>
                        <span className="text-muted-foreground">数据库</span>
                        <p className="font-medium">{systemDbConfig.database}</p>
                      </div>
                      <div>
                        <span className="text-muted-foreground">用户名</span>
                        <p className="font-medium">{systemDbConfig.username}</p>
                      </div>
                      <div>
                    ...
                    <p className="text-xs text-muted-foreground">
                      配置写入 .env 的 DATABASE_URL，修改后需重启应用服务后生效。
                    </p>
                    <Button variant="outline" size="sm" className="gap-1" onClick={startEditSystemDb}>
