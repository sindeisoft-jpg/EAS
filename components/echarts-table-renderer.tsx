...
export function EChartsTableRenderer({ 
  ...
  const formData = useMemo(() => {
    if (selectedRowIndex !== null && paginatedData[selectedRowIndex]) {
      return paginatedData[selectedRowIndex]
    }
    // 如果没有选中行，显示当前页第一条数据
    return paginatedData.length > 0 ? paginatedData[0] : null
              ...
              {columns.map((col, idx) => (
                ...
                >
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-sm font-semibold">{translateColumnName(col)}</span>
                  </div>
                </th>
              ))}
            </tr>
            ...
            {paginatedData.length === 0 ? (
              ...
              paginatedData.map((row, rowIdx) => (
                <tr 
                  ...
                  className={`border-b border-border/30 transition-colors duration-150 cursor-pointer ${
          ...
          <h4 className="text-base font-semibold text-foreground mb-4">
            ...
            {selectedRowIndex !== null && (
              <span className="text-sm font-normal text-muted-foreground ml-2">
                （第 {((currentPage - 1) * pageSize) + selectedRowIndex + 1} 条）
              </span>
            )}
          </h4>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {columns.map((col) => (
              <div key={col} className="space-y-2">
                ...
                <div className="p-3 rounded-lg border border-border/50 bg-muted/30 text-foreground min-h-[2.5rem] flex items-center">
                  <span className="break-words">
                    {formatCellValue(formData[col])}
                  </span>
                </div>
              </div>
            ))}
