...
import { EnhancedAgentSelector } from "./enhanced-agent-selector"
...
export function ChatInterface({ connections, llmConnections = [], agents = [], userId, organizationId, onSaveReport }: ChatInterfaceProps) {
  ...
  // 使用 ref 存储最新的 sessionStates，避免 getSessionState 的循环依赖
  const sessionStatesRef = useRef<Record<string, SessionState>>({})
  // 使用 ref 存储上一次保存的会话状态哈希，避免无限循环
  const lastSavedStateHashesRef = useRef<Map<string, string>>(new Map())
  ...
  // 直接从 sessionStates 获取，避免通过 getSessionState 导致的循环依赖
  const currentSessionState = useMemo(() => {
    ...
    return sessionStates[currentSessionId]
  ...
  // 同步搜索查询到 localStorage
  useEffect(() => {
    localStorage.setItem("chat_search_query", searchQuery)
  }, [searchQuery])
  ...
  // 用户从其他标签页/窗口返回时，重新检查并恢复当前会话的进行中任务
  useEffect(() => {
    const handleVisibilityChange = () => {
      if (document.visibilityState === "visible" && currentSessionId && !currentSessionId.startsWith("session_")) {
        checkAndRecoverTasks(currentSessionId)
      }
    }
    document.addEventListener("visibilitychange", handleVisibilityChange)
    return () => document.removeEventListener("visibilitychange", handleVisibilityChange)
  }, [currentSessionId, checkAndRecoverTasks])
  ...
  const loadSession = useCallback(async (sessionId: string) => {
    try {
      ...
      if (session && session.messages) {
        ...
        // 将合并后的消息转换为数组并排序
        let mergedMessages = sortMessages(Array.from(messageMap.values()))
        
        // 若会话仍在处理中且最后一条是用户消息，添加占位助手消息以恢复「思考中」气泡
        const isSessionProcessing = session.status === "processing" || !!session.currentTaskId
        const lastMsg = mergedMessages[mergedMessages.length - 1]
        if (isSessionProcessing && lastMsg?.role === "user") {
          const placeholderId = generateMessageId(sessionId, "assistant")
          if (!mergedMessages.some((m) => m.id === placeholderId)) {
            mergedMessages = sortMessages([
              ...mergedMessages,
              {
                id: placeholderId,
        ...
        // 加载完成，清除加载状态并标记为已加载（若会话仍在处理中则保留 isLoading）
        const stillProcessing = session.status === "processing" || !!session.currentTaskId
        if (!stillProcessing) {
          loadingSessionIdsRef.current.delete(sessionId)
          updateSessionState(sessionId, (state) => ({
            ...state,
  ...
  const createNewSession = useCallback(async () => {
    ...
    // 通知全局标题栏
    window.dispatchEvent(new CustomEvent("chat-creating-session"))
  ...
  // 监听全局标题栏的事件（必须在 createNewSession 定义之后）
  useEffect(() => {
    ...
    window.addEventListener("chat-search-query-change", handleSearchQueryChange as EventListener)
    window.addEventListener("chat-create-new-session", handleCreateNewSession)
    window.addEventListener("chat-clear-all", handleClearAll)
    
    return () => {
      window.removeEventListener("chat-search-query-change", handleSearchQueryChange as EventListener)
      window.removeEventListener("chat-create-new-session", handleCreateNewSession)
      window.removeEventListener("chat-clear-all", handleClearAll)
    }
  }, [createNewSession])
  ...
  const confirmClearAllSessions = async () => {
    ...
    // 通知全局标题栏
    window.dispatchEvent(new CustomEvent("chat-clearing-all"))
    try {
      const { apiClient } = await import("@/lib/api-client")
  ...
  // 为每个会话独立保存
  // 使用 useRef 跟踪上一次的状态，避免无限循环
  const prevSessionStatesRef = useRef<Record<string, SessionState>>({})
  const saveTimersRef = useRef<Map<string, NodeJS.Timeout>>(new Map())
  const sessionStatesRefForSave = useRef<Record<string, SessionState>>({})
  
  // 同步 sessionStates 到 ref，避免在 useEffect 中直接依赖 state
  useEffect(() => {
    sessionStatesRefForSave.current = sessionStates
  }, [sessionStates])
  
  // 使用独立的 useEffect，不依赖 sessionStates，而是通过 ref 访问
  // 使用定时器定期检查，而不是在每次渲染时检查
  // 提取依赖值，避免在依赖数组中使用可选链或对象属性访问
  const agentDatabaseId = useMemo(() => agentDatabase ? agentDatabase.id : '', [agentDatabase])
  const llmModel = useMemo(() => llmConfig.model, [llmConfig.model])
  const llmProvider = useMemo(() => llmConfig.provider, [llmConfig.provider])
  
  useEffect(() => {
    // 计算状态哈希，只在真正变化时才保存
    const getStateHash = (state: SessionState) => {
    ...
    // 检查并保存会话的函数
    const checkAndSaveSessions = () => {
      // 从 ref 获取最新的 sessionStates
      const currentSessionStates = sessionStatesRefForSave.current
      
      // 清理已删除会话的定时器
      const currentSessionIds = new Set(Object.keys(currentSessionStates))
      saveTimersRef.current.forEach((timer, sessionId) => {
        if (!currentSessionIds.has(sessionId)) {
          clearTimeout(timer)
          saveTimersRef.current.delete(sessionId)
          delete prevSessionStatesRef.current[sessionId]
        }
      })
      
      // 遍历所有会话状态
      Object.entries(currentSessionStates).forEach(([sessionId, sessionState]) => {
        // 只处理有消息的会话
        ...
        // 检查状态是否真正变化
        const currentHash = getStateHash(sessionState)
        const prevHash = prevSessionStatesRef.current[sessionId] 
          ? getStateHash(prevSessionStatesRef.current[sessionId])
          : null
        
        // 如果状态没有变化，跳过保存
        if (currentHash === prevHash && prevHash !== null) {
          return
        }
        
        // 清除之前的定时器
        const existingTimer = saveTimersRef.current.get(sessionId)
        if (existingTimer) {
          clearTimeout(existingTimer)
        }
        
        // 更新上一次的状态（使用深拷贝避免引用问题）
        prevSessionStatesRef.current[sessionId] = {
          messages: [...sessionState.messages],
          queryResults: { ...sessionState.queryResults },
          firstQueryResults: { ...sessionState.firstQueryResults },
        ...
        if (dbConnectionId) {
            ...
            if (isTemporaryId) {
              ...
              try {
                ...
                if (newSession && newSession.id && newSession.id !== sessionId) {
                  ...
                  // 使用函数式更新，避免触发依赖 sessionStates 的 useEffect
                  const currentState = sessionStatesRef.current[sessionId] || getSessionState(sessionId)
                  updateSessionState(newSession.id, () => currentState)
                  // 更新 prevSessionStatesRef，避免再次触发保存
                  prevSessionStatesRef.current[newSession.id] = { ...currentState }
                  delete prevSessionStatesRef.current[sessionId]
                  // 延迟更新 sessionStates，避免立即触发 useEffect
                  setTimeout(() => {
                    setSessionStates(prev => {
                      const newStates = { ...prev }
          ...
          // 存储定时器引用
          saveTimersRef.current.set(sessionId, saveTimer)
        }
      })
    }
    
    // 初始检查一次
    checkAndSaveSessions()
    
    // 设置定期检查（每5秒检查一次），而不是在每次渲染时检查
    const intervalId = setInterval(() => {
      checkAndSaveSessions()
    }, 5000);
    
    // 清理函数：组件卸载时清除所有定时器和间隔
    return () => {
      clearInterval(intervalId)
      saveTimersRef.current.forEach(timer => clearTimeout(timer))
      saveTimersRef.current.clear()
    }
    // 注意：不依赖 sessionStates，而是通过 ref 访问，避免无限循环
    // 只在关键依赖项变化时重新设置检查机制
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [selectedConnection, selectedAgentId, agentDatabaseId, llmModel, llmProvider, organizationId, userId, currentSessionId])
  ...
  if (llmConnections.length === 0) {
    return (
      <Card className="p-12 text-center">
        ...
        <Button onClick={() => (window.location.href = "/dashboard/models")}>前往大模型管理</Button>
      </Card>
    )
  }
  ...
  return (
    <div className="flex w-full h-[calc(100vh-3.5rem)] overflow-hidden">
      ...
      >
        ...
        >
          <h2 className="text-base font-semibold text-[#1f2937]">历史记录</h2>
        </div>
        ...
        {sortMessages(messages)
          ...
          >
            {message.role === "assistant" && (
              <div className="w-9 h-9 rounded-xl bg-gradient-to-br from-[#1e3a5f] to-[#2c5282] flex items-center justify-center flex-shrink-0 border border-[#1e3a5f]/30 shadow-sm transition-all duration-200 hover:shadow-md hover:scale-105">
                {message.content === PLACEHOLDER_ASSISTANT_MESSAGE ? (
                  <div className="relative w-5 h-5">
                    {/* 外层旋转圆环 */}
                    <div className="absolute inset-0 rounded-full border-2 border-white/20"></div>
                    {/* 中层旋转圆环 - 持续旋转 */}
                    <div className="absolute inset-0 rounded-full border-2 border-transparent border-t-white border-r-white animate-thinking-spin"></div>
                    {/* 内层脉冲圆点 */}
                    <div className="absolute inset-1.5 rounded-full bg-white/60 animate-thinking-pulse"></div>
                    {/* 中心点 */}
            ...
            {/* 用户消息：显示在气泡中，右侧对齐 - 商务风格 */}
            {message.role === "user" ? (
              <div className="flex-1 max-w-[85%] flex justify-end animate-fade-in-up group">
                <div className="rounded-xl px-5 py-3 pr-12 transition-all duration-200 hover:shadow-lg bg-gradient-to-br from-[#1e3a5f] to-[#2c5282] text-white shadow-md relative border border-[#1e3a5f]/20">
                  <div className="flex items-start gap-2">
                    <div className="flex-1 whitespace-pre-wrap text-[15px] leading-[1.7] text-white font-normal">
              ...
              <div className="flex-1 max-w-[85%] animate-fade-in-up group relative">
                <div className="w-full relative">
                  ...
                  {/* 助手消息气泡 - 商务风格 */}
                  <div className="bg-gradient-to-br from-[#f8f9fa] to-[#ffffff] rounded-xl px-5 py-4 border border-[#e5e7eb] shadow-sm hover:shadow-md transition-all duration-200">
                    <div className={`space-y-3 ${isErrorMessage(message) ? "text-red-600 dark:text-red-400" : "text-[#1f2937]"}`}>
                    {/* 使用Markdown渲染助手消息内容 */}
                    {(() => {
                      // 占位消息：渲染统一的“处理+思考”提示，避免与底部 loading 重复显示
                      if (message.content === PLACEHOLDER_ASSISTANT_MESSAGE) {
                        return (
                          <div className="bg-gradient-to-br from-[#f3f4f6] to-[#ffffff] rounded-lg px-4 py-2.5 border border-[#e5e7eb] shadow-sm animate-fade-in inline-block">
                            <div className="flex items-center gap-2">
                              {/* 流畅的弹跳动画 - 三个圆点依次弹跳，展现思考律动 */}
                              <div className="flex items-center gap-1.5">
                                <div className="w-2 h-2 rounded-full bg-[#1e3a5f] animate-thinking-bounce" style={{ animationDelay: '0ms' }}></div>
                                <div className="w-2 h-2 rounded-full bg-[#2c5282] animate-thinking-bounce" style={{ animationDelay: '200ms' }}></div>
                                <div className="w-2 h-2 rounded-full bg-[#3b82f6] animate-thinking-bounce" style={{ animationDelay: '400ms' }}></div>
                              </div>
                              <p className="text-sm text-[#374151] font-medium animate-thinking-text">
                      ...
                      // 错误消息：直接显示，不使用Markdown - 商务风格
                      if (isErrorMessage(message)) {
                        return (
                          <div className="whitespace-pre-wrap text-[15px] leading-[1.75] text-[#dc2626] font-medium">
                            {displayContent}
                          </div>
                        )
                      }
                      
                      // 普通消息：使用Markdown渲染（renderMarkdown内部会再次过滤）- 商务风格
                      return (
                        <div className="text-[15px] leading-[1.75] text-[#374151] font-normal">
                    ...
                    <div className="mt-4 pt-4 border-t border-border/30">
                      <ReportPreviewCard
                        ...
                        onExport={async (format) => {
                          ...
                          try {
                            if (format === "markdown") {
                              const { formatReportAsMarkdown } = await import("@/lib/report-formatter")
                              const markdown = formatReportAsMarkdown(message.metadata.aiReport)
                              ...
                              const { formatReportAsJSON } = await import("@/lib/report-formatter")
                              const json = formatReportAsJSON(message.metadata.aiReport)
