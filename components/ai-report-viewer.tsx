...
export function AIReportViewer({ 
  ...
  const renderSection = (section: ReportSection) => {
    ...
    return (
      <Card key={section.id} className="p-5 mb-4 bg-white border border-[#e5e7eb] shadow-sm rounded-xl hover:shadow-md transition-all duration-200">
        <div
        ...
        >
          <div className="flex items-center gap-3">
            ...
            <h3 className="text-lg font-semibold text-[#1f2937]">{section.title}</h3>
          </div>
          <Button variant="ghost" size="sm" className="text-[#6b7280] hover:text-[#1e3a5f] hover:bg-[#f3f4f6]">
            {isExpanded ? "收起" : "展开"}
          </Button>
        </div>
  ...
  const renderSectionContent = (section: ReportSection) => {
    switch (section.type) {
      case "text":
        return (
          <div className="prose max-w-none text-[#374151] text-[15px] leading-[1.75]">
          ...
          return (
            <div className="space-y-4">
              {chartData.charts.map((chart: any, index: number) => (
                <div key={index} className="border border-[#e5e7eb] rounded-xl p-4 bg-white shadow-sm">
              ...
              <table className="w-full border-collapse border border-[#e5e7eb] rounded-lg overflow-hidden">
                <thead>
                  <tr className="bg-gradient-to-r from-[#1e3a5f] to-[#2c5282]">
                    {tableData.columns.map((col: string) => (
                      <th key={col} className="border border-[#1e3a5f]/20 p-3 text-left text-white text-[14px] font-semibold">
                        {col}
                      </th>
                    ))}
                  </tr>
                ...
                <tbody>
                  {tableData.rows.slice(0, 20).map((row: any, rowIndex: number) => (
                    <tr key={rowIndex} className={rowIndex % 2 === 0 ? 'bg-white' : 'bg-[#f9fafb]'}>
                      {tableData.columns.map((col: string) => (
                        <td key={col} className="border border-[#e5e7eb] p-3 text-[14px] text-[#374151]">
                          {row[col] ?? ""}
                        </td>
                      ))}
                    </tr>
  ...
  const getSectionIcon = (type: ReportSection["type"]) => {
    switch (type) {
      case "chart":
        return <BarChart3 className="w-5 h-5 text-[#1e3a5f]" />
      case "table":
        return <TableIcon className="w-5 h-5 text-[#0d9488]" />
      case "ai_analysis":
        return <TrendingUp className="w-5 h-5 text-[#f59e0b]" />
      case "ai_summary":
        return <FileText className="w-5 h-5 text-[#475569]" />
      default:
  ...
  const reportContent = (
    <div className={`w-full ${modal ? 'h-full' : 'h-full'} flex flex-col`}>
      {/* 报告头部 - 商务风格 */}
      <Card className="p-6 mb-4 bg-gradient-to-br from-[#f8f9fa] to-[#ffffff] border border-[#e5e7eb] shadow-md rounded-xl">
        <div className="flex items-start justify-between">
          <div className="flex-1">
            <h1 className="text-2xl font-bold mb-2 text-[#1f2937]">{report.title}</h1>
            <p className="text-[#6b7280] mb-4 text-[15px] leading-relaxed">{report.goal}</p>
            <div className="flex items-center gap-4 text-sm text-[#6b7280] flex-wrap font-medium">
          ...
          <div className="flex gap-2 flex-wrap">
            {onShare && (
              <Button variant="outline" size="sm" onClick={onShare} className="border-[#d1d5db] text-[#374151] hover:bg-[#f3f4f6] hover:border-[#1e3a5f] rounded-lg">
