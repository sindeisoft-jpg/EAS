...
export class SQLExecutor {
  ...
  private static async executeSQLite(connection: DatabaseConnection, sql: string, startTime: number): Promise<QueryResult> {
    try {
      // 动态加载 better-sqlite3，避免构建时解析原生模块导致失败
      const { default: Database } = await import("better-sqlite3")

      // SQLite 数据库路径
      // 对于 SQLite，database 字段存储文件路径
      // 如果 database 是相对路径，需要转换为绝对路径
      let dbPath = connection.database
      
      if (!dbPath) {
        throw new Error("SQLite 数据库路径未指定")
      }
      ...
      try {
        ...
        if (rows.length > 0) {
          ...
          // 如果没有行，尝试从语句中获取列信息
          // 对于 SELECT 查询，尝试从 SQL 中提取列名
          const selectMatch = sql.match(/SELECT\s+(.+?)\s+FROM/i)
          if (selectMatch) {
            const selectClause = selectMatch[1].trim()
            if (selectClause !== "*") {
              // 提取列名（简化处理，假设是逗号分隔的列名）
              const columnNames = selectClause
                .split(",")
                .map((col) => col.trim().replace(/^`|`$/g, "").split(/\s+AS\s+/i)[0].trim())
      ...
      const executionTime = Date.now() - startTime
      console.error("[SQLExecutor] SQLite error:", error)
      return {
        columns: [],
        rows: [],
        rowCount: 0,
        executionTime,
        error: error.message || "SQLite 查询执行失败",
      }
    }
  }
