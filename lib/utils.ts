...
/**
 * 将查询结果的列名统一翻译为中文（columns 与 rows 中的 key 一致）
 * 用于 API 返回或展示前，保证表格表头为中文
 *
 * @param queryResult 查询结果 { columns: string[], rows: any[] }
 * @returns 翻译后的查询结果，列名与行数据的 key 均为中文
 */
export function translateQueryResult(queryResult: any): any {
  ...
  const translatedRows = queryResult.rows.map((row: any) => {
    ...
    Object.keys(row).forEach((originalCol) => {
      const translatedCol = columnMapping[originalCol] ?? translateColumnName(originalCol)
