    ...
    for (let idx = 0; idx < selectStatements.length; idx++) {
      ...
      if (selectMatch) {
        ...
        columns.forEach((col) => {
          ...
          // ✅ 检查是否是 "字符串常量 AS 别名" 格式（如 '固定值' AS label）
          // 先提取 AS 前的部分，检查是否是字符串常量
          const asIndex = trimmedCol.toUpperCase().indexOf(' AS ')
          if (asIndex > 0) {
            const beforeAs = trimmedCol.substring(0, asIndex).trim()
            if ((beforeAs.startsWith("'") && beforeAs.endsWith("'")) ||
                (beforeAs.startsWith('"') && beforeAs.endsWith('"'))) {
              // 这是字符串常量，提取别名但不验证字段
              const afterAs = trimmedCol.substring(asIndex + 4).trim()
              // 移除可能的引号
              const alias = afterAs.replace(/^['"]|['"]$/g, '').split(/\s|,/)[0]
              if (alias) {
                aliases.add(alias)
                if (isLastSelect) {
                  lastSelectAliases.add(alias)
