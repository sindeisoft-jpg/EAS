...
class ApiClient {
  ...
  /** 本系统自身使用的数据库（安装时配置的 DATABASE_URL），非对话用业务库 */
  async getSystemDatabase() {
    return this.request<{
      configured: boolean
      message?: string
      config: { type: string; host: string; port: number; database: string; username: string; password: string } | null
    }>("/settings/system-database")
  }

  async updateSystemDatabase(params: { host: string; port: number; database: string; username: string; password: string }) {
    return this.request<{
      success: boolean
      message: string
      warning?: string
